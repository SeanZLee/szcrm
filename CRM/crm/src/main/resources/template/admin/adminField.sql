#namespace("admin.field")
    #sql ("queryAddField")
    select field_id,field_name,name,type,options,is_null,"" as value,field_type,is_unique from 72crm_admin_field where label = ? order by sorting
    #end
   ...
   #end
   #sql ("fieldleadsview")
     create or replace view fieldleadsview as select %s batch_id as field_batch_id from 72crm_admin_fieldv as a force index(field_batch) inner join 72crm_admin_field as d on `a`.`field_id` = `d`.`field_id` %s where d.label = %s group by a.batch_id
   #end
   #sql ("fieldcustomerview")
     create or replace view fieldcustomerview  as select %s batch_id as field_batch_id from 72crm_admin_fieldv as a force index(field_batch) inner join 72crm_admin_field as d on `a`.`field_id` = `d`.`field_id`  %s where d.label = %s group by a.batch_id
   #end
   #sql ("fieldcontactsview")
     create or replace view fieldcontactsview as select %s batch_id as field_batch_id from 72crm_admin_fieldv as a force index(field_batch) inner join 72crm_admin_field d on `a`.`field_id` = `d`.`field_id`  %s where d.label = %s group by a.batch_id
   #end
   #sql ("fieldproductview")
     create or replace view fieldproductview as select %s batch_id as field_batch_id from 72crm_admin_fieldv as a force index(field_batch) inner join 72crm_admin_field as d on `a`.`field_id` = `d`.`field_id`  %s where d.label = %s group by a.batch_id
   #end
   #sql ("fieldbusinessview")
     create or replace view fieldbusinessview as select %s batch_id as field_batch_id from 72crm_admin_fieldv as a force index(field_batch) inner join 72crm_admin_field as d on `a`.`field_id` = `d`.`field_id`  %s where d.label = %s group by a.batch_id
   #end
   #sql ("fieldcontractview")
     create or replace view fieldcontractview as select %s batch_id as field_batch_id from 72crm_admin_fieldv as a force index(field_batch) inner join 72crm_admin_field as d on `a`.`field_id` = `d`.`field_id`  %s where d.label = %s group by a.batch_id
   #end
   #sql ("fieldreceivablesview")
     create or replace view fieldreceivablesview as select %s batch_id as field_batch_id from 72crm_admin_fieldv as a force index(field_batch) inner join 72crm_admin_field as d on `a`.`field_id` = `d`.`field_id`  %s where d.label = %s group by a.batch_id
   #end
#end
