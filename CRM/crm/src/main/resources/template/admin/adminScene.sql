#namespace("admin.scene")
    ...
    #sql ("queryCrmPageListByFieldType1")
        ...
        RIGHT JOIN (
          SELECT
            ...
            #for(field : fieldMap)
              ...
              #end
            #end
            FROM 72crm_admin_fieldv AS a RIGHT JOIN (SELECT batch_id FROM 72crm_crm_#(realm) WHERE c_uid=#para(host)
      ...
      ) as views WHERE 1=1 #for(query : queryList) #if(query.get("type")==2) #(query.get("sql")) #end #end
    #end
    ...
    #sql ("queryCrmPageListCount")
      SELECT count(1) FROM 72crm_crm_#(realm)
      WHERE 1=1
      #if(batchList&&batchList.size()>0)
          and batch_id in ( #for(str:batchList) #(for.index == 0 ? "" : ",") #para(str) #end)
      #end
    ...
    select *,(TO_DAYS(IFNULL((SELECT car.create_time FROM 72crm_admin_record as car where car.types = 'crm_customer' and car.types_id = views.customer_id ORDER BY car.create_time DESC LIMIT 1),create_time))+ CAST((SELECT value FROM 72crm_admin_config WHERE name= 'customerPoolSettingFollowupDays') as SIGNED) - TO_DAYS(NOW())) as pool_day,(select count(*) from 72crm_crm_business as a where a.customer_id = views.customer_id) as business_count
    #end
