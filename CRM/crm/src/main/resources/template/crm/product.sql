#namespace("crm.product")
    ...
    #sql("queryById")
       ...
       from 72crm_crm_product as a
       LEFT JOIN 72crm_crm_product_category as sc on sc.category_id = a.category_id
       where a.product_id = ?
    #end
    #sql("queryProductPageList")
      ...
      from
             (select sccp.product_id,scp.name as product_name,sccp.price,sccp.sales_price,sccp.num,sccp.discount,sccp.subtotal,c.name as category_name,scp.batch_id,scp.c_uid
