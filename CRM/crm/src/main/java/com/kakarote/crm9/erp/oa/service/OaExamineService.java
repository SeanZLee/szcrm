...
public class OaExamineService{
    ...
    private void transfer(List<Record> recordList){
        recordList.forEach(record -> {
            ...
            Long userId = BaseUtil.getUser().getUserId();
            if((BaseUtil.isSuperUser() && (examineStatus == 4 || examineStatus == 2)) || (createUserId.equals(BaseUtil.getUser().getUserId().intValue()) && (examineStatus == 4 || examineStatus == 2))){
                permission.put("isUpdate", 1);
            }else{
                permission.put("isUpdate", 0);
            }
            if(roles.contains(BaseUtil.getSuperRole()) && examineStatus != 1){
                permission.put("isDelete", 1);
            }else{
                permission.put("isDelete", 0);
            }
            if(((BaseUtil.isSuperUser() || userId.equals(record.getInt("create_user_id"))) && (examineStatus == 0 || examineStatus == 3)) && examineStatus != 4){
    ...
    public R setOaExamine(JSONObject jsonObject){
        ...
        if("0".equals(checkUserIds) && ! BaseUtil.isSuperUser(oaExamine.getCreateUserId().longValue())){
            checkUserIds = BaseUtil.getSuperUser().toString();
        }
    ...
    public R oaExamine(OaExamineLog nowadayExamineLog, Long nextUserId){
        ...
        if(status == 4){
            if(! examineRecord.getCreateUser().equals(auditUserId) && ! BaseUtil.isSuperUser()){
        ...
        if(status == 2){
            ...
            if(examineCategory.getExamineType() == 1){
                ...
                if(flag){
                    ...
                    if("0".equals(checkUserIds) && ! BaseUtil.isSuperUser(createUserId.longValue())){
                        checkUserIds = BaseUtil.getSuperUser().toString();
                    }
                }
            }
    ...
    public R queryExamineRecordList(Integer recordId){
        ...
        //判断是否有撤回权限
        if((auditUserId.equals(examineRecord.getLong("create_user")) || BaseUtil.isSuperUser(auditUserId)) && (examineStatus == 0 ||examineStatus == 3)){
            ...
            steps.forEach(step -> {
                if(step.getInt("step_type") == 1){
                    ...
                    if(logs != null && logs.size() > 0){
                        ...
                        if(r == null || r.size() == 0){
                            r = Db.find(Db.getSql("oa.examine.queryUserByUserIdAnd"), BaseUtil.getSuperUser());
                        }
                        step.set("userList", r);
                    }
                    ...
                    if(logs != null && logs.size() != 0){
                        ...
                        if(r == null && r.size() == 0){
                            r = Db.find(Db.getSql("admin.examineLog.queryUserByUserIdAnd"), BaseUtil.getSuperUser());
                        }
                        step.set("userList", r);
                    }
                }
