...
public class CrmBusinessService {
    ...
    public Record queryBusinessStatus(Integer businessId) {
        List<Record> statusList = Db.find(Db.getSql("crm.business.queryBusinessStatus"), businessId);
        Record business = Db.findFirst("select business_id,status_id,is_end,status_remark from 72crm_crm_business where business_id = ?",businessId);
    ...
    public List<Record> queryField(Integer businessId) {
        ...
        fieldList.add(new Record().set("field_name","product").set("name","产品").set("value",kv).set("formType","product").set("setting",new String[]{}).set("is_null",0).set("field_type",1));
        return fieldList;
    }
