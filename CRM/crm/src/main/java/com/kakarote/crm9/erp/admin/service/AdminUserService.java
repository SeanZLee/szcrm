...
public class AdminUserService {
    ...
    public R setUser(AdminUser adminUser, String roleIds) {
        ...
        if (adminUser.getUserId() == null) {
            if(!ReUtil.isMatch("^(?=.*[a-zA-Z])(?=.*\\d)(?=.*[~!@#$%^&*()_+`\\-={}:\";'<>?,.\\/]).{8,20}$", adminUser.getPassword())){
                return R.error("密码必须由 8-20位字母、数字、特殊符号线组成");
            }
            Integer count = Db.queryInt("select count(*) from 72crm_admin_user where username = ?", adminUser.getUsername());
            Record record = Db.findFirstByCache("hosts", "queryByPhone:"+adminUser.getUsername(), "SELECT * FROM `72crm_admin_hosts` where phone=?", adminUser.getUsername());
            ...
            bol =Db.tx(()->{
                Record recordHost=new Record().set("phone",adminUser.getUsername()).set("super",0).set("create_time",new Date()).set("host",BaseUtil.getHost().getStr("host"));
                return adminUser.save()&&Db.save("72crm_admin_hosts",recordHost);
            });
    ...
    public AdminUser resetUser() {
        ...
        ClusterRedis.use().setex(BaseUtil.getToken(), 360000, adminUser);
        return adminUser;
    }
    ...
    public R resetPassword(String ids, String pwd) {
        if(!ReUtil.isMatch("^(?=.*[a-zA-Z])(?=.*\\d)(?=.*[~!@#$%^&*()_+`\\-={}:\";'<>?,.\\/]).{8,20}$", pwd)){
            return R.error("密码必须由 8-20位字母、数字、特殊符号线组成");
        }
        for (String id : ids.split(",")) {
            ...
            BaseUtil.userExit(Long.valueOf(id));
        }
        return R.ok();
    }
    ...
    public R usernameEdit(Integer id,String username, String password){
        ...
        Record record = Db.findFirstByCache("hosts", "queryByPhone:"+username, "SELECT * FROM `72crm_admin_hosts` where phone=?",username);
        if (count > 0||record!=null) {
            return R.error("手机号重复！");
        }
        String oldUserName=adminUser.getUsername();
        record=Db.findFirstByCache("hosts", "queryByPhone:"+oldUserName, "SELECT * FROM `72crm_admin_hosts` where phone=?",oldUserName);
        adminUser.setUsername(username);
        adminUser.setMobile(username);
        adminUser.setPassword(BaseUtil.sign(username+password,adminUser.getSalt()));
        record.set("phone",username);
        boolean b=Db.update("72crm_admin_hosts",record)&&adminUser.update();
        if(!b){
            throw new RuntimeException();
        }
        CaffeineCache.ME.remove("hosts","queryByPhone:"+oldUserName);
        BaseUtil.userExit(adminUser.getUserId());
        return R.ok();
    }
