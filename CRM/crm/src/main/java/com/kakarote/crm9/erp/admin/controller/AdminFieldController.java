...
public class AdminFieldController extends Controller {
    ...
    public void information(@Para("types")Integer types,@Para("id")Integer id){
        ...
        if (types.equals(2)){
            ...
            auth = AuthUtil.isCrmAuth(AuthUtil.getCrmTablePara(CrmEnum.getSign(types)), id);
        }
     ...
     * 验证字段数据
     */
    @NotNullValidate(value = "val",message = "字段校验参数为空")
    @NotNullValidate(value = "types",message = "字段校验参数为空")
    @NotNullValidate(value = "fieldName",message = "字段校验参数为空")
    @NotNullValidate(value = "fieldType",message = "字段校验参数为空")
    public void verify(){
