...
//                    .add("detail_address", "详细地址", "text", settingArr)
            ...
            fieldUtil.add("name", "产品名称", "text", settingArr)
                    .add("category_name", "产品分类", "category", settingArr)
    ...
    public String getSubUserId(Integer userId,Integer deepness) {
        ...
        return StrUtil.isNotEmpty(ids.toString()) ? ids.toString() : ",0";
    }
    ...
    public R getCrmPageList(BasePageRequest basePageRequest){
        ...
        if(StrUtil.isEmpty(sortField) || StrUtil.isEmpty(orderNum)){
            ...
            kv.set("orderByKey", sortField).set("orderByType", orderNum).set("fieldType", fieldMap.get(sortField) != null ? fieldMap.get(sortField).getFieldType() : 1);
        }
        ...
        if(!type.equals(8) && ! type.equals(4) && ! BaseUtil.getSuperUser().equals(userId)){
        ...
        if(2 == type){
            ...
            selectSql = 1 == configType ? "select *,(TO_DAYS(IFNULL((SELECT car.create_time FROM 72crm_admin_record as car where car.types = 'crm_customer' and car.types_id = views.customer_id ORDER BY car.create_time DESC LIMIT 1),create_time))+ CAST((SELECT value FROM 72crm_admin_config WHERE name= 'customerPoolSettingFollowupDays' and c_uid='"+BaseUtil.getHost().getInt("id")+"') as SIGNED) - TO_DAYS(NOW())) as pool_day,(select count(*) from 72crm_crm_business as a where a.customer_id = views.customer_id) as business_count": "select *,(select count(*) from 72crm_crm_business as a where a.customer_id = views.customer_id) as business_count";
        }else if(6 == type){
        ...
        kv.set("select", selectSql).set("queryList", queryList).set("realm", realm).set("fieldMap", fieldMap).set("label", type == 8 ? 2 : type).set("host",BaseUtil.getCompanyId());
        ...
        resultJsonObject.put("dataCount", count);
    ...
    private boolean appendSqlCondition(Kv kv, Map<String, AdminField> fieldMap, List<JSONObject> queryList, JSONObject data) {
        ...
        for(JSONObject jsonObject : jsonObjectList){
            ...
            if(StrUtil.isNotEmpty(value) && ! isValid(value)){
                return false;
            }
            if(StrUtil.isNotEmpty(jsonObject.getString("start")) && ! isValid(jsonObject.getString("start"))){
                return false;
            }
            if(StrUtil.isNotEmpty(jsonObject.getString("end")) && ! isValid(jsonObject.getString("end"))){
                return false;
            }
            ...
            if("business_type".equals(formType)){
                ...
                conditions.append(" and address like '%").append(value).append("%'");
                sqlObject.put("type", 1);
            }else {
