...
public class CrmCustomerController extends Controller {
    ...
    public void queryById(@Para("customerId")Integer customerId){
        ...
        if(auth){renderJson(R.ok().put("data",new Record().set("dataAuth",0))); return; }
    ...
    public void addRecord(@Para("")AdminRecord adminRecord){
        boolean auth = AuthUtil.isPoolAuth(adminRecord.getTypesId());
        if(auth){renderJson(R.noAuth()); return; }
    ...
    private void export(List<Record> recordList) throws IOException{
        ...
        for (Record record : recordList){
            list.add(record.remove("c_uid","batch_id","create_user_id","customer_id","is_lock","owner_user_id","ro_user_id","rw_user_id","followup","field_batch_id").getColumns());
        }
