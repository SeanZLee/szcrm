...
public class CrmCustomerService {
    ...
    public R queryContacts(BasePageRequest<CrmCustomer> basePageRequest) {
        ...
        Integer pageType = basePageRequest.getPageType();
        if (0 == pageType) {
            return R.ok().put("data", Db.find(Db.getSqlPara("crm.customer.queryContacts",Kv.by("customerId",customerId))));
        } else {
            return R.ok().put("data", Db.paginate(basePageRequest.getPage(), basePageRequest.getLimit(), Db.getSqlPara("crm.customer.queryContacts",Kv.by("customerId",customerId))));
        }
    }
    ...
    public R updateRulesSetting(Integer dealDay,Integer followupDay,Integer type) {
        Db.update("update 72crm_admin_config set value = ? where name = 'customerPoolSettingDealDays'  AND c_uid = ?",dealDay,BaseUtil.getCompanyId());
        Db.update("update 72crm_admin_config set value = ? where name = 'customerPoolSettingFollowupDays'  AND c_uid = ?",followupDay,BaseUtil.getCompanyId());
        Db.update("update 72crm_admin_config set status = ? where name = 'customerPoolSetting'  AND c_uid = ?",type,BaseUtil.getCompanyId());
        return R.ok();
    }
