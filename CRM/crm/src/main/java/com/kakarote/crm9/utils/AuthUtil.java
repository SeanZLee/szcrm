...
//    }
    ...
    public static boolean isOaAuth(Integer type, Integer id){
        ...
        //超级管理员角色拥有最高权限
        if (roleIds.contains(BaseUtil.getSuperRole())||userId.equals(BaseUtil.getSuperUser())) {
            return false;
        }
        if(tablePara == null){
    ...
    public static boolean isPoolAuth(Integer customerId){
        ...
        if(ownerUserId==null){
            ...
            return AuthUtil.isCrmAuth(AuthUtil.getCrmTablePara(CrmEnum.CUSTOMER_TYPE_KEY.getSign()), customerId);
        }
    }
}
