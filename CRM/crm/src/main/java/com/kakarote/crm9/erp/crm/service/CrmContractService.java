...
public class CrmContractService {
    ...
    public R saveAndUpdate(JSONObject jsonObject) {
        ...
        if(crmContract.getStartTime()!=null&&crmContract.getEndTime()!=null){
            if(crmContract.getStartTime().getTime()>crmContract.getEndTime().getTime()){
                return R.error("结束时间应该大于开始时间");
            }
        }
